...
const Leave = () => {
  ...
  const isAdmin = false; // Change this to false to test employee view
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        {!isAdmin && !isDeleted && (
          <button
            ...
            className="btn btn-primary flex items-center gap-2 w-full sm:w-auto justify-center sm:ml-auto"
          >
      ...
      {!isAdmin && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5 mb-8">
          {leaveStats.map((s) => (
            ...
            >
              <div className="absolute left-0 top-0 bottom-0 w-1 bg-slate-100 group-hover:bg-cyan-500/70" />
              <div className="p-3 bg-slate-200 rounded-lg group-hover:bg-cyan-50 transition-colors duration-200">
                <s.icon className="w-5 h-5 text-slate-600 group-hover:text-cyan-600 transition-colors duration-200 " />
              </div>
              <div>
