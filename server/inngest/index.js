...
const autoChekOut = inngest.createFunction(
...
// Send reminder if leave application stays pending for 24 hours.
const leaveApplicationReminder = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if (leaveApplication?.status === "PENDING") {
      ...
      //SEND REMINDER LEAVE APPLICATION
      await sendEmail ({
...
// Check attendance at 11:30 AM IST (06:00 UTC) and identify absent employees.
const attenadanceReminderCorn = inngest.createFunction(
  { id: "attendance-reminder-corn", triggers: [{ cron: "0 0 6 * * *" }] },
  async ({ step }) => {
    ...
    if (absentEmployees.length > 0) {
      await step.run("send-reminder-emails", async () => {
       const emailPromises = absentEmployees.map((emp) =>{
        ...
        return absentEmployees.map((employee) => employee.email);
      });
    }
